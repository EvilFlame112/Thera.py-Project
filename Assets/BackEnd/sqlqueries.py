...
cur=conn.cursor()
def addtopatient(pid, namep, age, ailment,admsts, paytypep, status, contactp, amountp):
    cur.execute(f"INSERT INTO patient values({pid}, '{namep}', {age}, '{ailment}','{admsts}', '{paytypep}', '{status}', {contactp}, {amountp})")
    conn.commit()
...
def addtoemployee(eid,namee,job,doje,contacte,salarye):
    ...
    conn.commit()
def addtopharmacy(itcd,Mname,QTS,MRP):
    cur.execute(f"insert into pharmacy values({itcd},{Mname},{QTS},{MRP})")
    conn.commit()
def addtochdoctor(slno,chid,name, sp, dojch,contact,salary):
    cur.execute(f"insert into chdoctor values({slno},{chid},{name},{sp},{dojch},{contact},{salary})")
    conn.commit()
...
def purgeP():
    ...
    cur.execute("drop table chdoctor")
...
def uppatient(pid1, namep1, age1, ailment1,admsts, paytypep1, status1, contactp1, amountp1):
    cur.execute(f"update patient set Name='{namep1}',Age={age1},Ailment='{ailment1}',Admission_Status='{admsts}',Payment_Type='{paytypep1}',Payment_Status='{status1}',Contact={contactp1},Amount={amountp1} Where Patient_Id={pid1}")
    conn.commit()
...
def upemployee(eid1,namee1,job1,doje1,contacte1,salarye1):
    ...
    conn.commit()
def uppharmacy(itcd1,Mname1,QTS1,MRP1):
    cur.execute(f"update pharmacy set Item_Code={itcd1},Medicine_Name='{Mname1}',QTS={QTS1},MRP={MRP1}")
    conn.commit()
def upchdoctor(slno1,chid1,name1, sp1, dojch1,contact1,salary1):
    cur.execute(f"update chdoctor set Ch_Id={chid1},Name='{name1}',Specialization={sp1}, DOJ={dojch1}, Contact ={contact1},salary={salary1} where Slno={slno1}")
    conn.commit()
